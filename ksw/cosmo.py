...
class Cosmology:
    ...
    def add_lensing_bispectrum(self):
        ...
        c_ell_yphi = np.zeros((2, ells.size))
        c_ell_xz = np.zeros((3, ells.size))

        c_ell_yphi[0] = self.c_ell['lensed_scalar']['c_ell'][:,0]
        c_ell_yphi[1] = self.c_ell['lensed_scalar']['c_ell'][:,1]
    ...
    def get_lensing_bispectrum_factors(ells, c_ell_yphi, c_ell_xz):        
        ...
        ells : (nell,) array
            Multipoles.
        c_ell_yphi : (2, nell) array
            T-phi and E-phi power spectra.
        c_ell_xz : (3, nell) array
        ...
        # Note, the fact that we have the same number of unique factors
        # and bispectrum terms (nrule) is a concidence I think.
        rule = np.zeros((20, 3), dtype=np.int64)
        weights = np.ones((20, 3))
